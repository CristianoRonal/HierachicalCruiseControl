function [v_t,a_t,isdone] = Speed_replan(v0,a0,x0,vdes)
% 全局速度规划（1km全长）
% v0当前时刻车辆的实际行驶速度(m/s)
...
elseif x0<x_lowmu_end
    vmax(1:round(x_lowmu_end-x0)/ds)=vdes;
    vmax(round(x_lowmu_end-x0)/ds+1:end)=vlim;
else
    isdone=1;
end
...
% v_t=v_t(2);
a_t=as_to_at_1(a_s,s_slide,v0,delta_t,ds);
% a_t=a_t(2);
v_t=v0+a_t*delta_t;
...
function a_t = as_to_at_1(a_s,s_slide,v0, delta_t,ds)
s=0:ds:s_slide;
...
% 更新速度
% v_t = v_t + a_t * delta_t;  % 基于加速度和时间步长更新速度，得到delta_t时间后的速度

% 计算新的位移
s_new = s_current + v_t * delta_t;  % 基于当前速度和时间步长更新位移，得到delta_t时间后的位移
