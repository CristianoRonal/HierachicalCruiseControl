...
class ActorNetwork(nn.Module):
    ...
    def sample_action(self, a_acc, a_plan, action_range=1.):
        # normal = Normal(0, 1)
        # random_action=action_range*normal.sample( (self.action_dim,) )
        low_a  = min(a_acc, a_plan)
        high_a = max(a_acc, a_plan)
        prior_action = np.random.uniform(low_a, high_a, (self.action_dim,))
        prior_action = 2 * (prior_action - low_a) / (high_a - low_a) - 1
...
def plot_test(ego_v, ego_dp, ego_x, i_episode):
    ...
    plt.tight_layout()
    plt.savefig(f'./figure/ddpg_128_eps{i_episode}')
    plt.close(fig)
...
if __name__ == '__main__':
    ...
    hidden_dim = 128
    explore_steps = 20000  # for random exploration
    batch_size = 64

    replay_buffer_size=1e6
    replay_buffer = ReplayBuffer(replay_buffer_size)
    model_path='./model/ddpg_128'
        ...
        for i_episode in range (max_episodes):
            ...
            for step in range(max_steps):
                if frame_idx > explore_steps:
                    ...
                    action = alg.policy_net.sample_action(a_acc, a_plan, action_range=1.)
    ...
    if args.test:
        ...
        for i_episode in range (test_episodes):
            ...
            plot_test(ego_v, ego_dp, ego_x, i_episode)
